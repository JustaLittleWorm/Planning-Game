...
class CalendarApp(tk.Tk):   
    ...
    def create_widgets(self):
        ...
        calendar_label = tk.Label(self.root, text="TAKVİM", font=("Times New Roman", 20, "bold"))
        calendar_label.pack(pady=10)
        
        current = dt.datetime.now()
        # Create the current date label
        self.date_label = tk.Label(self.root, text=current.strftime("%A, %B %d, %Y"), font=("Times New Roman", 12))
        ...
        calendar_info_label = tk.Label(self.root, text="Etkinlikleri Görmek İçin Tıklayın.", font=("Arial", 8))
        ...
        self.year_label = tk.Label(navigation_frame, text="Yıl", font=("Arial", 12))
        ...
        self.month_label = tk.Label(navigation_frame, text="Ay", font=("Arial", 12))
        ...
        self.selected_event_label_d = tk.Label(self.root, text="Etkinlik Seçilmedi.", font=("Arial", 12))
        ...
        buttons_frame.pack()
        # Create buttons
        create_button = tk.Button(buttons_frame, text="Etkinlik Oluştur", command=self.create_event)
        create_button.grid(row=0,column=0,pady=5)

        update_button = tk.Button(buttons_frame, text="Etkinlik Güncelle", command=self.update_event)
        update_button.grid(row=0,column=1,pady=5)

        delete_button = tk.Button(buttons_frame, text="Etkinlik Sil", command=self.delete_event)
        delete_button.grid(row=1,column=0,pady=5)
        
        quit_button = tk.Button(buttons_frame, text="Takvimden Çık", command=self.quit_calendar)
        quit_button.grid(row=1,column=1,pady=5)

    def quit_calendar(self):
        self.save_events_to_json(self.tckno)
        print("Takvimden Çıkılıyor.")
        super().destroy()
    ...
    def generate_calendar(self):
        ...
        self.year_label.config(text=f"Yıl : {self.current_year}", font=("Arial", 8, "bold"))
        self.month_label.config(text=f"Ay : {self.current_month}", font=("Arial", 8, "bold"))

    def create_event(self):
        if self.selected_date:
            time_picker = tk.Toplevel()
            time_picker.title("Zamanı Seçin")
            time_picker.geometry("200x150")

        # Saat seçmek için bir zaman seçici widget'ı oluştur
            selected_time = tk.StringVar()
            time_picker_label = tk.Label(time_picker, text="Saati Girin:(hh:dd):", font=("Arial", 14))
        ...
        # Create the create window
                ...
                create_window.title("Etkinlik Oluştur.")
                create_window.geometry("400x450")

        # Create labels and entry boxes for window
                event_label = tk.Label(create_window, text="Etkinlik Tarihi:", font=("Arial", 14))
                ...
                event_label = tk.Label(create_window, text="Etkinlik Saati:", font=("Arial", 14))
                event_label.pack(pady=10)
                ...
                event_label = tk.Label(create_window, text="Etkinlik Oluşturma Zamanı:", font=("Arial", 14))
                event_label.pack(pady=10)
                event_label = tk.Label(create_window, text=dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), font=("Arial", 14))
                event_label.pack(pady=5)
            
                event_label = tk.Label(create_window, text="Etkinlik Adı:", font=("Arial", 14))
                event_label.pack(pady=10)

                event_entry = tk.Entry(create_window, font=("Arial", 14))
                event_entry.pack(pady=5)

                desc_label = tk.Label(create_window, text="Etkinlik Tarifi:", font=("Arial", 14))
        ...
        # Create save button
                save_button = tk.Button(create_window, text="Kaydet", command=lambda: self.add_event(self.selected_date, event_entry.get(), event_time, desc_entry.get(), create_window, creation_time))
                save_button.pack(pady=5)

        # Create cancel button
                cancel_button = tk.Button(create_window, text="İptal", command=create_window.destroy)
            ...
            time_picker_button = tk.Button(time_picker_gride, text="Kaydet", command=save_time)
            time_picker_button.grid(row=0, column=0, padx=5, pady=5)
            
            time_picker_button = tk.Button(time_picker_gride, text="İptal", command=time_picker.destroy)
            time_picker_button.grid(row=0, column=1, padx=5, pady=5)
            
        else:
            tk.messagebox.showwarning("Tarih Seçilmedi", "Lütfen Takvimden tarihi seçiniz.")
    ...
    def add_event(self, date, event_name,time, event_description, create_window, creation_time):
        ...
        tk.messagebox.showinfo("Etkinlik Oluşturudu.", "Etkinlik başarıyla oluşturuldu.")
           
    def delete_event(self):
        sorted_events = sorted(self.events[self.selected_date].items(), key=lambda x: dt.datetime.strptime(x[1]["time"], "%H:%M"))
        
        if tk.messagebox.askyesno("Silmeyi Onayla", "Etkinliği silmeyi onaylıyor musun ?"):
            del self.events[self.selected_date][sorted_events[self.event_listbox.curselection()[0]][0]]
            self.save_events_to_json(self.tckno)
            self.show_events(self.selected_date)
            self.generate_calendar()
            tk.messagebox.showinfo("Etkinlik Silindi", "Etkinlik başarıyla silindi.")
    ...
    def update_event(self):
        ...
        # Get the selected event from the listbox
            ...
            # Get the event details
                ...
                # Create labels and entry boxes for window
                event_label = tk.Label(update_window, text="Etkinlik tarihi", font=("Arial", 14))
                event_label.pack(pady=10)
                ...
                event_label = tk.Label(update_window, text="Etkinlik saati", font=("Arial", 14))
                event_label.pack(pady=10)
                event_time_ = tk.Entry(update_window, font=("Arial", 14))
                event_time_.pack(pady=5)
                event_time_.insert(0, event_time)
                
                event_label = tk.Label(update_window, text="Etkinlik Adı:", font=("Arial", 14))
                event_label.pack(pady=10)
                ...
                desc_label = tk.Label(update_window, text="Etkinlik  Tarifi", font=("Arial", 14))
                desc_label.pack(pady=10)
